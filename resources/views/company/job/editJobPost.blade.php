...
<div class="col-sm-12 col-lg-8 col-xl-9">
    <form action="{{ route('updatePostJob') }}" method="post">
        @csrf
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_input form-group">
                        ...
                        <input type="date" class="form-control @error('deadline') is-invalid @enderror" name="deadline" value="{{ $job->deadline }}">
                        @error('deadline')
                        <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="my_profile_select_box form-group">
                        ...
                        <select class="form-control @error('job_type') is-invalid @enderror selectpicker"  name="job_type">
                            ...
                            @foreach($jobTypes as $jobType)
                                <option value="{{ $jobType->id }}"  {{ ($job->job_type == $jobType->id ? "selected":"") }}>{{ $jobType->title }}</option>
                            @endforeach
                        </select>
                        @error('job_type')
                        <span class="invalid-feedback" role="alert">
                ...
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_input form-group">
                        ...
                        <input type="number" min="0" class="form-control @error('min_exp_year') is-invalid @enderror" name="min_exp_year" value="{{ $job->min_exp_year }}">
                        @error('min_exp_year')
                        <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_input form-group">
                        ...
                        <input type="number" min="0" class="form-control @error('max_exp_year') is-invalid @enderror" name="max_exp_year" value="{{ $job->max_exp_year }}">
                        @error('max_exp_year')
                        <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="col-md-6 col-lg-5">
                    <div class="my_profile_input form-group">
                        ...
                        <input type="number" min="0" id="minSalary" class="form-control @error('min_salary') is-invalid @enderror" name="min_salary" value="{{ $job->min_salary }}" {{ $job->is_negotiable == '1' ? 'disabled' : '' }}>
                        @error('min_salary')
                        <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="col-md-6 col-lg-5">
                    <div class="my_profile_input form-group">
                        ...
                        <input type="number" min="0" id="maxSalary" class="form-control @error('max_salary') is-invalid @enderror" name="max_salary" value="{{ $job->max_salary }}" {{ $job->is_negotiable == '1' ? 'disabled' : '' }}>
                        @error('max_salary')
                        <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="col-md-6 col-lg-2">
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" id="customCheck1" name="is_negotiable" value="1" {{ $job->is_negotiable == '1' ? 'checked' : ''}}>
                        <input type="hidden" id="hiddenNegotiable" name="is_negotiable" value="0" {{ $job->is_negotiable == '1' ? 'disabled' : '' }}>
                ...
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_select_box form-group">
                        ...
                        <select class="form-control @error('salary_terms') is-invalid @enderror selectpicker" name="salary_terms" id="salaryTerms" {{ $job->is_negotiable == '1' ? 'disabled' : '' }}>
                            <option value="">Select Salary Terms</option>
                            <option value="PER_YEAR" {{ ($job->salary_terms == 'PER_YEAR' ? "selected":"") }}>Per Year</option>
                            <option value="PER_MONTH" {{ ($job->salary_terms == 'PER_MONTH' ? "selected":"") }}>Per Month</option>
                        </select>
                        @error('salary_terms')
                        <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                ...
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_select_box form-group">
                        ...
                        <select class="form-control @error('currency') is-invalid @enderror selectpicker" name="currency" id="currency" {{ $job->is_negotiable == '1' ? 'disabled' : '' }}>
                            <option value="">Select Currency</option>
                            @foreach($currencies as $currency)
                                <option value="{{ $currency->id }}" {{ ($job->currency == $currency->id ? "selected":"") }}>{{ $currency->name }} ({{ $currency->code }})</option>
                            @endforeach
                        </select>
                        @error('currency')
                        <span class="invalid-feedback" role="alert">
                    ...
                    <div class="my_profile_select_box form-group">
                        ...
                        <select class="form-control @error('job_category') is-invalid @enderror selectpicker" name="job_category">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ ($job->job_category == $category->id ? "selected":"") }}>{{ $category->category_name }}</option>
                            @endforeach
                        </select>
                        @error('job_category')
                        <span class="invalid-feedback" role="alert">
                    ...
                    <div class="my_profile_select_box form-group">
                        ...
                        <select class="form-control @error('job_industry') is-invalid @enderror selectpicker" name="job_industry">
                            ...
                            @foreach($industries as $industry)
                                <option value="{{ $industry->id }}" {{ ($job->job_industry == $industry->id ? "selected":"") }}>{{ $industry->industry_name }}</option>
                            @endforeach
                        </select>
                        @error('job_industry')
                        <span class="invalid-feedback" role="alert">
                    ...
                    <div class="my_profile_select_box form-group">
                        ...
                        <select class="form-control @error('job_qualification') is-invalid @enderror selectpicker" name="job_qualification">
                            ...
                            @foreach($jobQualifications as $jobQualification)
                                <option value="{{ $jobQualification->id }}" {{ ($job->job_qualification == $jobQualification->id ? "selected":"") }}>{{ $jobQualification->qualification_name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_input form-group">
                        ...
                        <input type="number" class="form-control" name="min_age" min="0" value="{{ $job->min_age }}">
                    </div>
                </div>
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_input form-group">
                        <label>Maximum Age</label><br>
                        <input type="number" class="form-control" name="max_age" min="0" value="{{ $job->max_age }}">
                    </div>
                </div>
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_input form-group">
                        <label>Vacancy<span class="required"> *</span></label>
                        <input type="number" min="0" class="form-control @error('position_count') is-invalid @enderror" name="position_count" value="{{ $job->position_count }}">
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="custom-control custom-checkbox is-visa" style="margin-top: -48px;">
                        <input type="checkbox" class="custom-control-input" id="customCheck2" name="is_visa_sponsor" value="1" {{ ($job->is_visa_sponsor == '1') ? 'checked' : '' }}>
                        <input type="hidden" id="hiddenVisa" name="is_visa_sponsor" value="0" {{ ($job->is_visa_sponsor == '1') ? 'disabled' : '' }}>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="custom-control custom-checkbox is-relocation" style="margin-top: -48px;">
                        <input type="checkbox" class="custom-control-input" id="customCheck3" name="is_relocation" value="1" {{ ($job->is_relocation == '1') ? 'checked' : '' }}>
                        <input type="hidden" id="hiddenRelocation" name="is_relocation" value="0" {{ ($job->is_relocation == '1') ? 'disabled' : '' }}>
                        <label class="custom-control-label" for="customCheck3">Relocation</label>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4">
                    <div class="custom-control custom-switch">
                        <input type="checkbox" class="custom-control-input" id="customSwitch4" name="is_published" value="1" {{ ($job->is_published == '1') ? 'checked' : '' }}>
                        <input type="hidden" id="hiddenPublished" name="is_published" value="0" {{ ($job->is_published == '1') ? 'disabled' : '' }}>
                ...
                <div class="col-md-6 col-lg-6">
                    <div class="my_profile_select_box form-group">
                        ...
                        <select class="form-control @error('city_id') is-invalid @enderror selectpicker" id="city" name="city_id">
                            @php
                                $cities = \App\City::where('country_id', $job->country_id )->orderBy('name','asc')->get();
                            @endphp
                ...
                <div class="col-lg-12">
                    <div class="my_resume_textarea">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" rows="9" name="submission_instruction">{{ $job->submission_instruction }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
...
@section('myJs')
    ...
    <script>
        ...
        @if($job->submission_type != ''){
            $('#applyMethod').val('{{ $job->submission_type }}');
        }
        @endif
    </script>
